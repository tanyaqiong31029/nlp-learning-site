...
| 💬 NLP 概念 | 19 个概念分三组递进：**经典基础**（词袋、TF-IDF、词向量、Token、Transformer、上下文窗口……）、**LLM 时代新概念**（RAG、提示工程、后训练与 RLHF、推理模型、Agent、多模态）、**评测素养**（Benchmark 与榜单、LLM-as-a-Judge、数据污染、标注一致性 κ、机器翻译质量评估），全部用大白话与类比讲解，附科研者必读的「幻觉守则」 |
| 🧪 实操项目 | **浏览器内直接运行**的五个实验：TF-IDF 文本相似度（热力矩阵）、Burrows-Delta 文体计量（经典 z-score 算法）、语料统计实验台、bigram 下一词预测器（亲手体会"语言模型 = 条件概率"）、标注一致性实验台（Cohen's κ，校验 LLM-as-a-Judge 的标准做法）；可修改语料实时看结果，各附 Python 版脚本（可复制 / 下载 .py） |
| 📖 资源知识库 | GitHub 优质开源学习资源的结构化知识库：**43 个项目 × 9 大分类**（含「LLM 评测」与「算法工程师进阶」分类），每条含简介、推荐理由、星标与最近更新时间，支持分类筛选与关键词搜索 |
...
- ✅ **学习进度标记**：62 个可勾选项（知识点 / 路线步骤 / 项目），按模块与整体汇总进度条，保存在浏览器 `localStorage`，页脚可一键重置
...
│   ├── experiments.js      # 五个在线实验（相似度 / 文体计量 / 语料统计 / bigram 语言模型 / 标注一致性）
│   ├── content.js          # 知识库与前沿动态渲染（含实时看板网络护栏）
│   ├── app.js              # hash 路由、学习进度、页面导航、启动引导
│   └── data/
│       ├── knowledge.js    # 知识库数据（43 个资源 × 9 分类，GitHub API 每日核验）
...
- 中文按单字切分的 TF-IDF、字符级 Burrows' Delta（1982）、bigram 语言模型、Cohen's κ 一致性系数均为纯 JS 实现，方便对照学习
